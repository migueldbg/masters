...
int ADCBoard::UnpackEvent(UInt_t size)
{
  ...
  //  printf ("Channel Mask %x\n",fADCEvent->GetAcceptedChannelMask() & 0xFFFF);
  long int acceptedch = fADCEvent->GetAcceptedChannelMask() & 0xFFFF;
  int ns = fADCEvent->GetAcceptedChannelMask()>>16 & 0xFFFF;
  //Override the number of samples read from the event mask
  if (fCustomNSamples)
    ns = fNSamples;

  for(int ic=0;ic<ADCEVENT_NCHANNELS;ic++){
     if ( acceptedch & (0x1 << ic) ) {
...
void ADCBoard::ForceNumberOfSamplesTo(int nsamples)
{
  //Overrides the number of samples from the event header
  //(to be used with care!)
  if (nsamples > ADCEVENT_NSAMPLES)
    {
      std::cout << "Cannot have wf of " << nsamples << " samples" << std::endl;
      std::cout << "Maximum allocated memory is " << ADCEVENT_NSAMPLES << std::endl;
      exit(1);
    }
  if (nsamples < 1)
    {
      std::cout << "Invalid choice of nsamples" << std::endl;
      std::cout << "Going to ignore the setting " << std::endl;
      return;
    }

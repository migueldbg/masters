...
{ 
  ...
  for (map<int,vector<double>* >::iterator it=wfs.begin(); 
      ...
      else //non-SiPM, non-LSci
	{
      ...
      if (fVerbosity)
	printf ("b %f q %f integral %f ser %f channel %d\n", 
		fBaseline->m(),fBaseline->q(),
		fIntegral->charge(),ser[channumber],
		channumber);  
	       ...
	       RDconfig::kLSci)
	{
      ...
      if (fVerbosity)
	printf("charge %f f90 %f f90_tot %f\n", charge[channumber],
	       f90[channumber], f90_tot);
  ...
  // baseline
  fBaseline->DoIt(&wf_all, cfg["baseline_start"], 
  		  cfg["baseline_stop"]);
  ...
  RDClusterization *cc = new 
    RDClusterization(wa_all, (int) cfg["baseline_stop"], nsamples, cfg["cluster_thr"], (int) cfg["peak_sigma"], 
  ...
  if (n_cluster)
      ...
      for (size_t n = 0; n < n_cluster; n++) 
	{
	  ...
	  fIntegral->DoIt(&wf_all, 0, (double) fBaseline->mean(),
			   ch_start, aCluster->stop, 
			   ch_start + (int) cfg["f90_stop"]);
	  ...
	  for (map<int,vector<double>* >::iterator it=wfs.begin(); 
		...
		{		  
		  ...
		  */
		  ...
		  RDconfig::kSiPMTopOFF)
		aCluster->charge_top.push_back(0.);
	      else if 
		(RDconfig::GetInstance()->GetChannelType(channumber) == 
		  RDconfig::kSiPMBottom)
		{
		  if (ser[channumber] > 0)

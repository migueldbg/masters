...
int GenerateAllHist(int run, int number_divisions, Double_t f90_min, Double_t f90_mid, Double_t f90_max, Double_t max_charge_run,
  ...
  std::array<std::string, 3> file_suffix = {"", "MC_ER2", "MC_NR2"};
  ...
  if (gSystem -> AccessPathName(Form("hist_%d2.root", run))){
    std::cout << "The " << Form("hist_%d2.root", run) << " file does not exist. Creating it..." << std::endl;
  } else {
    std::cout << "Opening file: " << Form("hist_%d2.root", run) << std::endl;
  }

  TFile *hist_file = new TFile(Form("hist_%d2.root", run), "UPDATE");
...
void CompareSimulation(int run, int number_divisions, bool hist_exist = false, Double_t f90_min = 0.1, Double_t f90_mid = 0.4, Double_t f90_max = 0.7,
                       Double_t max_charge_run = 2000., Double_t max_charge_MC_ER = 28000., Double_t max_charge_MC_NR = 9800.){
  ...
  ER_canvas1 -> SaveAs("ER_ratio12.pdf"); ER_canvas2 -> SaveAs("ER_ratio22.pdf");
  NR_canvas1 -> SaveAs("NR_ratio12.pdf"); NR_canvas2 -> SaveAs("NR_ratio22.pdf");

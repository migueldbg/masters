...
void GetKinematics( double *masses, double BeamEnergy,  std::vector<double> &fE3,  std::vector<double> &fTheta3,  std::vector<double> &fE4,  std::vector<double> &fTheta4, bool SelForward)
{
  ...
  Double_t m2=masses[1];           //Proton mass
  ...
  //  ------------
  Double_t m3=masses[2];           //7Be mass
  Double_t m4=masses[3];            //Neutron mass
...
void Summary(TTree *t,  int iF )
{
  ...
  // [isetting]
  double SiTel_Range_i[3][2]={ { 18.5, 20.5 }, { -100, -100 }, { 17.5, 19.5 } }; // LowBe blob
  ...
  cout << "********************************************" << endl;
  cout << "Li band: " << IsLiBand << endl;
  cout << "Be band: " << IsBeBand << endl;
  ...
  IsBe=Form("%s && %s >%6.2lf &&  %s < %6.2lf ", 
	    IsBeBand.c_str(), SiEnergy.c_str(), SiTel_Range[0], SiEnergy.c_str(),  SiTel_Range[1] );
 

  IsMonitor=Form("%s>%6.2lf &&  %s < %6.2lf ", 
		 MonitEnergy.c_str(), SiMon_Range[0], MonitEnergy.c_str(),  SiMon_Range[1] );

...
RECODIR=/storage/DATA-02/darkside/red/reco/rm3reco/naples/camp_VIII
file_name="run_${runumber}"
# Replace here, in order to process on the LNS DAQ server
#SCRIPTDIR=/home/pandola/red-daq-light/src/Level1/script
LEVEL1DIR=/storage/local/home/darkside/sanfilippo/red-daq-light-ori/src/Level1
SCRIPTDIR=/storage/local/home/darkside/sanfilippo/red-daq-light-ori/src/Level1/script 

cd $LEVEL1DIR
#./script/makelist_multi_lns.sh $runumber
if [ $2 == laser ]
	then
	./RedLevel1 -l list/$list_name -m cfg/$ch_mapping.cfg -c cfg/$globalnumber.cfg -o $RECODIR/$file_name.root --laser
	echo " Reco $file_name done! "
else
   ./RedLevel1 -l list/$list_name -m cfg/$ch_mapping.cfg -c cfg/$global_name.cfg -e ser/$ser_name.cfg -o $RECODIR/$file_name.root 
echo " Reco $file_name done! ";
fi
